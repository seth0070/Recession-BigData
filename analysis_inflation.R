# =============================================================================
# Inflation Google Trends vs. Gold Price Analysis
# =============================================================================
# This script investigates whether rising global online concern about inflation
# — captured through Google Trends search intensity for "Inflation" — is
# associated with subsequent increases in gold prices.
#
# Gold is a classic inflation hedge. If heightened public concern about
# inflation (as expressed through search behaviour) reliably *precedes*
...
#     File: gold-300.xls  (HTML table, Feb 2001 – Jun 2025)
#   - Google Trends data for "Inflation": Google Trends (global, monthly)
#     File: multiTimeline (4).csv  (exported from trends.google.com,
#                                   Jan 2004 – Mar 2026)
#
# Analysis steps:
...
# 1b. Parse Google Trends CSV for "Inflation" (multiTimeline (4).csv)
#     File has two preamble lines before the real header.
trends_raw  <- read.csv("multiTimeline (4).csv", skip = 2)
colnames(trends_raw)[colnames(trends_raw) != "Month"] <- "Inflation_Trend"
...
trends <- data.frame(
  ...
  Inflation_Trend = trends_raw$Inflation_Trend
)

cat("Google Trends (Inflation) data:", nrow(trends), "monthly observations\n")
...
print(summary(df$Inflation_Trend))

# ── 6. Time-series visualisations ─────────────────────────────────────────────
dir.create("output", showWarnings = FALSE)

png("output/inflation_time_series.png", width = 900, height = 1100, res = 110)
...
# Panel 3: Google Trends for "Inflation"
plot(df$Date, df$Inflation_Trend,
     type = "l", col = "#B22222", lwd = 2,
     xlab = "Date", ylab = "Search Index (0-100)",
     main = "Google Trends: \"Inflation\" Search Intensity (Global)",
     xaxt = "n", ylim = c(0, 100))
axis(1, at = year_breaks, labels = format(year_breaks, "%Y"))
polygon(c(df$Date[1], df$Date, tail(df$Date, 1)),
        c(0, df$Inflation_Trend, 0),
        col = adjustcolor("#B22222", alpha.f = 0.3), border = NA)

invisible(dev.off())
cat("Saved: output/inflation_time_series.png\n")

# ── 7. Contemporaneous correlation & hypothesis test ─────────────────────────
cat("\n=== Hypothesis Test 1: Contemporaneous Correlation ===\n")
cat("H0: cor(Inflation_Trend[t], Gold_Return[t]) = 0\n")
cat("H1: cor(Inflation_Trend[t], Gold_Return[t]) != 0  (two-sided)\n\n")
...
png("output/inflation_scatter_contemporaneous.png", width = 750, height = 580, res = 110)
par(mar = c(4.5, 4.5, 3.5, 1))
lm_contemp <- lm(Gold_Return ~ Inflation_Trend, data = df)
plot(df$Inflation_Trend, df$Gold_Return,
     col  = adjustcolor("#B22222", alpha.f = 0.5),
     pch  = 19, cex = 0.85,
     xlab = "Inflation Google Trends Index",
     ylab = "Gold Monthly Return (%)",
     main = "Google Trends \"Inflation\" vs. Gold Monthly Return (Same Month)")
...
invisible(dev.off())
cat("Saved: output/inflation_scatter_contemporaneous.png\n")
...
ccf_result <- ccf(df$Inflation_Trend, df$Gold_Return,
...
png("output/inflation_ccf_plot.png", width = 850, height = 480, res = 110)
par(mar = c(4.5, 4.5, 3.5, 1))
...
barplot(bar_heights,
        ...
        main      = "CCF: Google Trends \"Inflation\" -> Gold Monthly Return",
...
invisible(dev.off())
cat("Saved: output/inflation_ccf_plot.png\n")
...
png("output/inflation_scatter_lag1.png", width = 750, height = 580, res = 110)
par(mar = c(4.5, 4.5, 3.5, 1))
plot(df_lag$Trends_Lag1, df_lag$Gold_Return,
     col  = adjustcolor("#B22222", alpha.f = 0.5),
     pch  = 19, cex = 0.85,
     xlab = "Inflation Google Trends Index (1-month lag)",
     ylab = "Gold Monthly Return (%)",
     main = "Inflation Trends[t-1] vs. Gold Return[t]")
abline(lm_lag1, col = "#DAA520", lwd = 2)
mtext(sprintf("beta = %.4f,  p (one-sided) = %.4f", beta1, p_one_sid),
      side = 3, line = 0.3, cex = 0.85)
invisible(dev.off())
cat("Saved: output/inflation_scatter_lag1.png\n")

# ── 10. Granger causality test ────────────────────────────────────────────────
granger_test <- function(y, x, lag_order) {
  n_full <- length(y)
  if (n_full <= 2 * lag_order + 1) return(list(F = NA, p = NA))
...
cat("H0: Inflation Trends does NOT Granger-cause gold returns\n\n")

for (k in 1:3) {
  gc <- granger_test(df$Gold_Return, df$Inflation_Trend, lag_order = k)
  cat(sprintf("Lag %d: F = %.4f, p = %.4f  =>  %s\n",
    k, gc$F, gc$p,
    ifelse(gc$p < 0.05, "Granger causality detected", "No Granger causality")))
}
...
cat("   - inflation_time_series.png\n")
cat("   - inflation_scatter_contemporaneous.png\n")
cat("   - inflation_ccf_plot.png\n")
cat("   - inflation_scatter_lag1.png\n")
